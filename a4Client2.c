...
main(int argc, char **argv) 
{
 ...
 f = fopen("a4Client2Log.txt", "a");
 while (1) {
 strcat(sendline, "update classics set author = '*** UNKNOWN ***'  where author = 'Mark Twain'");
  ...
  sleep(2);
  strcpy(sendline,"");
  strcat(sendline, "update classics set author = 'Mark Twain'  where author = '*** UNKNOWN ***'");
  send(sockfd, sendline, strlen(sendline), 0);
  if (recv(sockfd, recvline, MAXLINE,0) == 0){
   //error: server terminated prematurely
   perror("The server terminated prematurely"); 
